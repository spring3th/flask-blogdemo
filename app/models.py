...
class Role(db.Model):
    ...
    def insert_roles():
        roles = {
            'User_Comment': (Permission.FOLLOW |
                             Permission.COMMENT, True),
            'User_WriteArticles':(Permission.FOLLOW |
                                  Permission.COMMENT |
...
class User(UserMixin, db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if self.role is None:
            if self.email == current_app.config['SIKA_ADMIN']:
...
class Post(db.Model):
    ...
    digest = db.Column(db.String(256))
    ...
    def generate_fake(count=100):
        ...
        for i in range(count):
            ...
            p = Post(title=forgery_py.lorem_ipsum.sentences(randint(1, 5)),
                     digest=forgery_py.lorem_ipsum.sentences(randint(1, 5)),
